...
% nohup matlab -nodesktop -nodisplay -nosplash <runLbfgsForwDiff.m >runLbfgsForwDiff.out 2>&1 &
...
% first put runLbfgsForwDiff.m in the working directory of the `minFunc` library
addpath(genpath(pwd));

%%
startRun = tic;
randSeed = 20190819; % for reproducibility
funcName = @popSFuncDiscus_;
...
for d = 1 : length(funcDims)
    ...
    options.numDiff = 1; % forward-differencing
...
save('runLbfgsForwDiff.mat', 'optRes', 'funcDims', '-v7.3');
